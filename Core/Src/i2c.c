...
void I2C_Transmit(uint8_t addr, uint8_t* p_buf, uint32_t len){
    ...
    HAL_I2C_Master_Transmit_DMA(&hi2c2, addr, p_buf, len);
    osSemaphoreAcquire(mutex_i2c_busyHandle, osWaitForever);
...
void I2C_Receive(uint8_t addr, uint8_t* p_buf, uint32_t len){
    osSemaphoreAcquire(mutex_i2c_idleHandle, osWaitForever);
    HAL_I2C_Master_Receive_DMA(&hi2c2, addr+ 1, p_buf, len);
    osSemaphoreAcquire(mutex_i2c_busyHandle, osWaitForever);
    osSemaphoreRelease(mutex_i2c_idleHandle);
}
